...
function createEmptyServiceRecord(input = {}) {
  ...
  return {
    ...
    og_image_url: String(input.og_image_url || "/assets/logo.png").trim(),
    pricing_min_inr: 0,
    pricing_max_inr: 0,
...
function buildAbsoluteAssetUrl(req, rawUrl) {
  if (!rawUrl) return `${req.protocol}://${req.get("host")}/assets/logo.png`;
...
async function getOrCreateDefaultTemplate() {
  const fieldsConfig = {
    ...
    date: { x: 88, y: 182, fontSize: 12, fontStyle: "normal", align: "center" },
    venue: { x: 148.5, y: 164, fontSize: 12, fontStyle: "normal", align: "center" },
